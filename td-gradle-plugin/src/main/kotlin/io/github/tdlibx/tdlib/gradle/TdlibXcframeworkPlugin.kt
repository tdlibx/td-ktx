...
class TdlibXcframeworkPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        val ext = project.extensions.create(
            "tdlibXcframework", TdlibXcframeworkExtension::class.java, project
    ...
    private fun configureMultiplatform(project: Project, ext: TdlibXcframeworkExtension) {
        ...
        val nativeBinaries = project.extensions.findByType(KotlinMultiplatformExtension::class.java)
            ?.targets
            ?.filterIsInstance<KotlinNativeTarget>()
            .orEmpty()
        val includedTargets = ext.includeTargets.orElse(emptyList()).get()

        val unpackTaskProvider = project.tasks.register(
        ...
        for (kt in nativeBinaries) {
            ...
            for (binary in kt.binaries) {
                ...
                val libraryIdentifier = when (kt.konanTarget.name) {
                    "ios_arm64" -> "ios-arm64"
                ...
                binary.linkerOpts(
                    ...
                    "-rpath", dylibDir.get().asFile.absolutePath,
                    "-ltdjson",
                    "-lz", "-lc++"
                )
